...
from xkool_site.components.poi.constant import ANJUKE_SECOND_HAND_COMMUNITY_NAME_LIST_TO_ADD, \
    ANJUKE_NEW_COMMUNITY_NAME_LIST_TO_ADD, ANJUKE_UPDATE_HEADER_LIST
from xkool_site.components.poi.dao.anjuke_dao import format_anjuke_new_community_raw_data, \
    format_anjuke_second_hand_community_raw_data
...
def consolidate_form(second_hand_community_data, new_community_data):
    ...
    to_be_appended = pd.DataFrame([[np.nan] * column_num] * row_num, columns=ANJUKE_SECOND_HAND_COMMUNITY_NAME_LIST_TO_ADD)
...
def merge_community_raw_data(second_hand_community_data, new_community_data):
    ...
    for i in range(len(second_hand_community_data)):
        ...
        if not new_community_data[new_community_data['community_name'] == name].empty:
            ...
            for new_name in ANJUKE_SECOND_HAND_COMMUNITY_NAME_LIST_TO_ADD:
                second_hand_community_data[new_name][i] = new_community_data.at[idx, new_name]
    # 把重叠的部分从新楼盘数据集中去除
...
def get_anjuke_ready_data(ready_data_file_path):
...
def process_and_save(city_name):
    ...
    ready_data = get_anjuke_ready_data(ready_data_file_path)
